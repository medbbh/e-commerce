...
const ProductCardItem = ({ product }) => {
  ...
  return (
    <div className="rounded overflow-hidden shadow-lg bg-white dark:bg-gray-800 transition-transform duration-300 hover:scale-105">
      <div className="relative">
        ...
        {images.length > 1 && (
          <>
            <button
              onClick={prevImage}
              className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-50 rounded-full p-1 transition hover:bg-opacity-75"
            >
              <ChevronLeft size={20} />
            </button>
            <button
              onClick={nextImage}
              className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-50 rounded-full p-1 transition hover:bg-opacity-75"
            >
      ...
      <div className="p-4">
        <h3 className="font-bold text-xl mb-2 text-gray-800 dark:text-white">{product.name}</h3>
        ...
        <div className="flex justify-between items-center">
          ...
          {/* Conditionally render "Add" or "Remove" */}
          {isInCart ? (
            <button
              onClick={() => deleteItem(cartItem.cart_id)}
